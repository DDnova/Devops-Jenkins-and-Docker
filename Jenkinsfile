  ...
  stages {
    ...
    stage('Deploy to ECS') {
      steps {
        ...
        sh "aws ecs update-service --cluster ${CLUSTER_NAME} --service ${SERVICE_NAME}  --desired-count ${DESIRED_COUNT} --task-definition ${TASK_DEFINITION_NAME}:${env.BUILD_ID} --force-new-deployment  --requires-compatibilities 'fargate'"
        }
    }
