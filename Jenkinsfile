    ...
    stage('Deploy to ECS') {
      steps {
            // prepare task definition file
                sh """sed -e "s;%REPOSITORY_URI%;${REPOSITORY_URI};g" -e "s;%SHORT_COMMIT%;${SHORT_COMMIT};g" -e "s;%TASK_DEFINITION_NAME%;${TASK_DEFINITION_NAME};g" -e "s;%SERVICE_NAME%;${SERVICE_NAME};g" -e "s;g" taskdef_template.json > taskdef_${SHORT_COMMIT}.json"""
                script {
