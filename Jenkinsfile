  ...
  stages {
    ...
    stage('Deploy to ECS') {
      steps {
        ...
        sh "aws ecs register-task-definition --family ${TASK_DEFINITION_NAME} --requires-compatibilities 'FARGATE'  --container-definitions '[{\"name\":\"${IMAGE_REPO_NAME}\",\"image\":\"${REPOSITORY_URI}:${IMAGE_TAG}\",\"portMappings\":[{\"containerPort\":3000}],\"essential\":true,\"memoryReservation\":128}]'"
